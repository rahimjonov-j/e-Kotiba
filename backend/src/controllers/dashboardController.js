...
import { processWithGemini } from "../services/aiService.js";
...
export const getDashboard = async (req, res, next) => {
  try {
    ...
    const [todayRemindersResult, upcomingMeetingsResult, missedRemindersResult, expensesResult, activeResult] = await Promise.all([
      safeArrayQuery(
        supabaseAdmin
        .from("reminders")
        .select("*")
    ...
    const degraded = [todayRemindersResult, upcomingMeetingsResult, missedRemindersResult, expensesResult, activeResult].some(
      (result) => result.degraded
    );
    const errors = [todayRemindersResult, upcomingMeetingsResult, missedRemindersResult, expensesResult, activeResult]
    ...
    if (env.geminiApiKey) {
      try {
        ...
        const recommendationRaw = await processWithGemini({ 
          rawText: JSON.stringify(stats), 
          mode: "dashboard_insights" 
        });
        recommendations = recommendationRaw.cleaned_text || recommendations;
      } catch {
